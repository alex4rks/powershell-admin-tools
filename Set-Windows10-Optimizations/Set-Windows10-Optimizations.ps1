...
####################################
#
# 1. Apply HKCU Settings to Registry
#
Write-Host "1. Apply HKCU Settings to Registry"
# Load default user registry
# Push-Location C:\Windows\system32
reg load "hklm\temp" "C:\Users\Default\NTUSER.DAT" # >$null
if ($?)
{
...
# reg add "hklm\temp\Control Panel\Desktop" /v "MenuShowDelay" /t REG_DWORD /d "200" /f >$null
...
# reg delete "hklm\temp\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "TaskbarSmallIcons" /f >$null 2>$null

# Hide titles in taskbar
# reg delete "hklm\temp\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v "TaskbarGlomLevel" /f >$null 2>$null
...
########################
#
# 2. Apply HKLM Settings
#
Write-Host "2. Apply HKLM Settings"
...
# 5.1 Remove Capabilities
Write-Host "5.1 Remove Capabilities"
...
#############################
#
# 6. Disable Features & Other
#
Write-Host "6. Disable Features & Other"

# 6.1. Disable OneDrive
Write-Host "6.1. Disable OneDrive"
...
# Ex.: http://www.scconfigmgr.com/2016/08/03/customize-pinned-items-on-taskbar-in-windows-10-1607-during-osd-with-configmgr/
